import { Component, ElementRef, NgZone, OnInit, Optional, ViewChild } from '@angular/core';
import { Geolocation, Position } from '@capacitor/geolocation';
import { ModalController, Platform, NavController, IonRouterOutlet } from '@ionic/angular';
...
const sub = this.auth.onAuthStateChanged(async (user)=>{
this.database.getEarnings(user.uid).subscribe(async (d)=>{
  this.earnings = d.Earnings;
  this.currentState = d.onlineState;
...
async Navigate(){
  if (this.platform.is('android')) {
    ...
    window.open('geo://' + this.LatLng.lat + ',' + this.LatLng.lng + '?q=' + this.D_LatLng.lat + ',' + this.D_LatLng.lng + '(' + await this.geocode.getAddress(this.LatLng.lat, this.LatLng.lng) + ')', '_system');
  }

  if (this.platform.is('ios')) {
    // tslint:disable-next-line: max-line-length
    window.open('maps://?q=' + await this.geocode.getAddress(this.LatLng.lat, this.LatLng.lng) + '&saddr=' + this.LatLng.lat + ',' + this.LatLng.lng + '&daddr=' + this.D_LatLng.lat + ',' + this.D_LatLng.lng, '_system');
  }
